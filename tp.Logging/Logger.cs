namespace tp.Logging
{
    ...
    /// all <see cref="LogListener"/> objects subscribed to it
    /// by calling the <see cref="LogListener.ProcessLog"/>() of each one
    /// passing an <see cref="LogEntry"/> object that represents the log information.
    /// </para>
    /// <para>
    /// The Logger doesn't actually logs the information in any medium.
    /// Instead it relies in that some one of its <see cref="LogListener"/> subscribers
    /// is capable of doing that.
    /// </para>
    /// <para>
    /// Any thread can call any of the <c>Log()</c> methods of the Logger.
    /// </para>    
    /// <para><strong>CAUTION</strong></para>
    /// <para>The Logger calls its Listeners asynchronously, that is from inside a thread.</para>
    /// <para>Thus Listeners should synchronize their <see cref="LogListener.ProcessLog"/>() call. Windows.Forms Controls need to check if InvokeRequired.</para>
    /// <para>Another solution to this problem is to use <see cref="LogListenerSynced"/> log listener.</para>   
    /// </summary>
    static public class Logger
    {
        ...
        /// <summary>
        /// Returns a string representation of a specified entry as json text.
        /// </summary>
        static public string GetAsJson(LogEntry Entry)
        {
            LogRecord LR = new LogRecord(Entry);
            return LogLib.Serialize(LR);
        }
        /// <summary>
        /// Returns a string representation of a specified entry as a string list.
        /// </summary>
        ...
        /// <summary>
        /// Returns a string representation of a specified entry as a single line.
        /// </summary>
